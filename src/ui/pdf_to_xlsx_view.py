...
class ConvertWorker(QThread):
    ...
    def __init__(self, input_path, output_path, extract_all_pages):
        super().__init__()
        self.input_path = input_path
        self.output_path = output_path
        self.extract_all_pages = extract_all_pages
    
    def run(self):
        try:
            with PdfToXlsxConverter(self.input_path) as converter:
                result = converter.convert(self.output_path, self.extract_all_pages)
            self.finished.emit(result)
        except Exception as e:
            self.error.emit(str(e))


class PdfToXlsxView(QWidget):
    """View for converting PDF to XLSX."""
    ...
    def _setup_ui(self):
        ...
        self.convert_btn = QPushButton("📊  Convert to XLSX")
    ...
    def _convert_pdf(self):
        ...
        output_path, _ = QFileDialog.getSaveFileName(
            self, "Save XLSX File", default_name, "Excel Files (*.xlsx)"
        )
        ...
        extract_all_pages = self.extract_all_check.isChecked()
    ...
    def _on_convert_finished(self, result):
        ...
        message = f"✅ PDF converted to XLSX! {table_count} table(s) extracted. Size: {output_mb:.2f} MB\nSaved to: {self.output_path}"
