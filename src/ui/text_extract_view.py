from PySide6.QtWidgets import (
    ...
    QLabel, QFrame, QTextEdit, QCheckBox, QScrollArea, QRadioButton, QLineEdit, QButtonGroup
)
...
class TextExtractView(QWidget):
    """Modern text extraction view."""
    ...
    def _setup_ui(self):
        """Set up the text extract view UI."""
        ...
        self.result_section.setMinimumHeight(200)
        layout.addWidget(self.result_section, 0)
    ...
    def _create_header():
        ...
        title = QLabel("Extract Text from PDF")
        title.setObjectName("sectionTitle")
        layout.addWidget(title)

        subtitle = QLabel("Extract and save text content from PDF documents")
    ...
    def _create_options_section(self):
        ...
        self.layout_checkbox = QCheckBox("Preserve text layout")
        self.layout_checkbox.setObjectName("layoutCheckbox")
        layout.addWidget(self.layout_checkbox)

        return container

    def _create_result_section(self):
        """Create the result text display section."""
        container = QFrame()
        container.setProperty("class", "file-section")
        layout = QVBoxLayout(container)
        layout.setContentsMargins(20, 20, 20, 20)
        ...
        result_header = QLabel("EXTRACTED TEXT")
        result_header.setProperty("class", "list-header")
    ...
    def _create_actions(self):
        ...
        self.save_btn.setProperty("class", "secondary-button")
    ...
    def extract_text(self):
        ...
        try:
            ...
            with TextExtractor(self.input_file) as extractor:
                if self.all_pages_radio.isChecked():
                    text = extractor.extract_all_text(preserve_layout=preserve_layout)
                else:
                    ...
                    text = extractor.extract_page_text(page_numbers, preserve_layout=preserve_layout)
            
            if text:
                self.text_display.setPlainText(text)
                self.result_section.show()
                self.save_btn.setEnabled(True)
                self._show_status(f"✅ Text extracted successfully! {len(text)} characters extracted.", "success")
            else:
                self.text_display.clear()
                self.result_section.hide()
                self.save_btn.setEnabled(False)
                self._show_status("⚠️ No text found in the PDF.", "error")
    ...
    def save_text(self):
        ...
        if not self.text_display.toPlainText():
            self._show_status("⚠️ No text to save. Extract text first.", "error")
            return

        output_file, _ = QFileDialog.getSaveFileName(
            self, "Save Extracted Text", "extracted_text.txt", "Text Files (*.txt)"
        )
        if not output_file:
            return
        ...
        try:
            ...
            self._show_status(f"✅ Text saved successfully to: {output_file}", "success")
        except Exception as e:
            self._show_status(f"❌ Failed to save text: {str(e)}", "error")

    def _apply_styles(self):
        """Apply futuristic styles to text extract view."""
        self.setStyleSheet("""
            TextExtractView {
            ...
            QCheckBox#layoutCheckbox::indicator {
                width: 20px;
                height: 20px;
                border: 2px solid #00d9ff;
                border-radius: 4px;
            ...
            QCheckBox#layoutCheckbox::indicator:checked {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #00d9ff, stop:1 #00b8d4);
            }
            
            QCheckBox#layoutCheckbox::indicator:hover {
                border-color: #00f0ff;
            }
            ...
            QTextEdit#textDisplay QScrollBar:vertical {
                background: #0a0e27;
                width: 12px;
                border: 1px solid #00d9ff;
                border-radius: 6px;
            ...
            QTextEdit#textDisplay QScrollBar::handle:vertical {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #00d9ff, stop:1 #00b8d4);
                border-radius: 5px;
                min-height: 20px;
            ...
            QTextEdit#textDisplay QScrollBar::handle:vertical:hover {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #00f0ff, stop:1 #00d9ff);
            }
            
            QTextEdit#textDisplay QScrollBar::add-line:vertical,
            QTextEdit#textDisplay QScrollBar::sub-line:vertical {
                height: 0px;
            }
            
            QTextEdit#textDisplay QScrollBar::add-page:vertical,
            QTextEdit#textDisplay QScrollBar::sub-page:vertical {
                background: none;
            }
