from PySide6.QtWidgets import (
    ...
    QLabel, QFrame, QScrollArea, QLineEdit, QRadioButton, QButtonGroup
)
...
class RemovePasswordWorker(QThread):
    """Worker thread for removing password."""
    finished = Signal(dict)
    error = Signal(str)

    def __init__(self, input_file, output_file, password=None):
        super().__init__()
    ...
    def run(self):
        try:
            with PDFEncryptor(self.input_file) as encryptor:
                result = encryptor.remove_password(self.output_file, self.password)
            self.finished.emit(result)
        except Exception as e:
            self.error.emit(str(e))


class RemovePasswordView(QWidget):
    ...
    def _create_header():
        ...
        title = QLabel("Remove Password Protection")
        title.setObjectName("sectionTitle")
        layout.addWidget(title)

        subtitle = QLabel("Decrypt password-protected PDF files")
    ...
    def _create_options_section(self):
        ...
        options_header = QLabel("DECRYPTION METHOD")
        options_header.setProperty("class", "list-header")
        layout.addWidget(options_header)

        self.method_group = QButtonGroup(self)
        
        self.with_password_radio = QRadioButton("I know the password")
        self.with_password_radio.setObjectName("pageRadio")
        self.with_password_radio.setChecked(True)
        self.with_password_radio.toggled.connect(self._on_method_changed)
        self.method_group.addButton(self.with_password_radio)
        layout.addWidget(self.with_password_radio)
        
        self.password_input = QLineEdit()
        self.password_input.setPlaceholderText("Enter the user or owner password")
        self.password_input.setObjectName("pagesInput")
        self.password_input.setEchoMode(QLineEdit.EchoMode.Password)
        layout.addWidget(self.password_input)

        help_with = QLabel("💡 Enter either the user password or owner password to decrypt the PDF.")
        help_with.setObjectName("helpLabel")
        help_with.setWordWrap(True)
        layout.addWidget(help_with)
        
        self.without_password_radio = QRadioButton("I don't know the password (attempt recovery)")
        self.without_password_radio.setObjectName("pageRadio")
        self.without_password_radio.toggled.connect(self._on_method_changed)
        self.method_group.addButton(self.without_password_radio)
        layout.addWidget(self.without_password_radio)

        self.recovery_label = QLabel()
        self.recovery_label.setObjectName("helpLabel")
        self.recovery_label.setWordWrap(True)
        self.recovery_label.setText("⚠️ Password recovery will attempt to remove encryption without the password. This only works for PDFs with weak or no owner password protection. Success is not guaranteed.")
        self.recovery_label.setEnabled(False)
    ...
    def _create_actions(self):
        ...
        self.process_btn = QPushButton("🔓  DECRYPT PDF")
    ...
    def _on_method_changed(self):
        ...
        with_password = self.with_password_radio.isChecked()
        self.password_input.setEnabled(with_password)
        self.recovery_label.setEnabled(not with_password)
    ...
    def _load_file(self, file):
        ...
        try:
            ...
            if self.total_pages == 0:
                self._show_status("⚠️ Cannot read encrypted PDF metadata. File will be processed.", "info")
                self.total_pages = 1
            ...
            if self.total_pages > 0:
                ...
                self.page_info_label.setText("Encrypted PDF (page count unavailable)")
    ...
    def process_pdf(self):
        ...
        if with_password:
            ...
            if not password:
                self._show_status("⚠️ Please enter the password.", "error")
                return
        else:
            password = None

        output_file, _ = QFileDialog.getSaveFileName(
            self, "Save Decrypted PDF", "decrypted.pdf", "PDF Files (*.pdf)"
        )
        if not output_file:
            return
        ...
        self.worker = RemovePasswordWorker(self.input_file, output_file, password)
    ...
    def _on_process_complete(self, result):
        ...
        self._show_status(f"✅ PDF decrypted successfully! {result['page_count']} pages processed.", "success")
        self.process_btn.setText("🔓  DECRYPT PDF")
    ...
    def _apply_styles(self):
        ...
        self.setStyleSheet("""
            ...
            QLabel#helpLabel:disabled {
                color: #5a5f7a;
                border-left-color: #3a3f5a;
            }
