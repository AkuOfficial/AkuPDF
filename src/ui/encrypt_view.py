...
class EncryptWorker(QThread):
    ...
    def __init__(self, input_file, output_file, operation, password, owner_password=None):
        super().__init__()
        ...
        try:
            with PDFEncryptor(self.input_file) as encryptor:
                if self.operation == "add":
                    result = encryptor.add_password(self.output_file, self.password, self.owner_password)
                else:
...
class EncryptView(QWidget):
    """PDF encryption/decryption view."""
    ...
    def _create_header():
        ...
        title = QLabel("PDF Encryption")
        title.setObjectName("sectionTitle")
        layout.addWidget(title)

        subtitle = QLabel("Add or remove password protection from PDF files")
    ...
    def _create_options_section(self):
        ...
        self.add_password_radio = QRadioButton("Add password protection")
        self.add_password_radio.setObjectName("pageRadio")
        self.add_password_radio.setChecked(True)
        self.add_password_radio.toggled.connect(self._on_operation_changed)
        self.operation_group.addButton(self.add_password_radio)
        layout.addWidget(self.add_password_radio)
        ...
        self.owner_password_input.setPlaceholderText("Owner password (optional)")
        ...
        self.remove_password_radio = QRadioButton("Remove password protection")
        self.remove_password_radio.setObjectName("pageRadio")
        self.remove_password_radio.toggled.connect(self._on_operation_changed)
        self.operation_group.addButton(self.remove_password_radio)
        layout.addWidget(self.remove_password_radio)
        
        self.decrypt_password_input = QLineEdit()
        self.decrypt_password_input.setPlaceholderText("Enter current password")
        self.decrypt_password_input.setObjectName("pagesInput")
        self.decrypt_password_input.setEchoMode(QLineEdit.EchoMode.Password)
        self.decrypt_password_input.setEnabled(False)
    ...
    def _on_operation_changed(self):
        ...
        is_add = self.add_password_radio.isChecked()
        self.password_input.setEnabled(is_add)
        self.owner_password_input.setEnabled(is_add)
        self.decrypt_password_input.setEnabled(not is_add)
        self.process_btn.setText("🔒  ENCRYPT PDF" if is_add else "🔓  DECRYPT PDF")
    ...
    def process_pdf(self):
        """Process PDF encryption/decryption."""
        self._hide_status()
        ...
        if is_add:
            ...
            if not password:
                self._show_status("⚠️ Please enter a password.", "error")
                return
            owner_password = self.owner_password_input.text().strip() or None
            operation = "add"
            default_name = "encrypted.pdf"
        else:
            password = self.decrypt_password_input.text().strip()
            if not password:
                self._show_status("⚠️ Please enter the current password.", "error")
                return
            owner_password = None
            operation = "remove"
            default_name = "decrypted.pdf"
        ...
        self.worker = EncryptWorker(self.input_file, output_file, operation, password, owner_password)
    ...
    def _on_process_complete(self, result):
        ...
        is_add = self.add_password_radio.isChecked()
        action = "encrypted" if is_add else "decrypted"
        self._show_status(f"✅ PDF {action} successfully! {result['page_count']} pages processed.", "success")
        self.process_btn.setText("🔒  ENCRYPT PDF" if is_add else "🔓  DECRYPT PDF")
